...
class CookieManager:
    def __init__(self, redis_host=REDIS_HOST, redis_port=REDIS_PORT, redis_db=REDIS_DB):
        ...
        self.cooke_ready_hash = "ctrip_ck_hash"
        self.cookie_ready = "ctrip_ck"       # 可立即使用的 cookie
        self.cookie_cooldown = "ctrip_ck_cooldown" # 正在冷却的 cookie（zset）


    def get_valid_cookie(self) -> str:
        """
        从 Redis 获取一个可用 cookie（无则等待）
        """
        while True:
            # Step 1: 尝试从 ready 队列获取
            cookie = self.redis.spop(self.cookie_ready)
            if cookie:
                logger.info(f"✅ 取到可用 cookie: ...")
                return cookie

            # Step 2: 检查是否有冷却完成的 cookie
            now = time.time()
            ready_cookies = self.redis.zrangebyscore(self.cookie_cooldown, 0, now)

            if ready_cookies:
                cookie = random.choice(ready_cookies)
                # 从冷却池删除，并加入 ready 池
                self.redis.zrem(self.cookie_cooldown, cookie)
                logger.info(f"♻️ 冷却完成 cookie 恢复可用...")
                return cookie

            # Step 3: 如果都没有，则等待一会儿
            logger.debug("⏳ 暂无可用 cookie，等待 5 秒重试...")
            time.sleep(5)

    def mark_cookie_used(self, cookie: str):
        """
        将 cookie 标记为使用中 → 放入冷却池
        """
        cooldown = random.randint(420, 600)
        next_ready_ts = time.time() + cooldown
        self.redis.zadd(self.cookie_cooldown, {cookie: next_ready_ts})
        logger.info(f"🕒 Cookie {cookie[:20]}... 进入冷却 {cooldown}s")

    @staticmethod
    def delete_hash_by_value(redis_conn, hash_key, target_value):
        # 获取 Hash 的所有字段和值
        all_data = redis_conn.hgetall(hash_key)
        ...
        # 删除匹配的字段
        if fields_to_delete:
            redis_conn.hdel(hash_key, *fields_to_delete)

        return len(fields_to_delete)


    def remove_invalid_cookie(self, cookie: str):
        """
        删除失效 cookie
        """
        self.redis.srem(self.cookie_ready, cookie)
        self.redis.zrem(self.cookie_cooldown, cookie)
        # self.delete_hash_by_value(self.redis, self.cooke_ready_hash, cookie)
        logger.warning(f"❌ 已移除失效 cookie: {cookie[:20]}...")
...
class Scheduler:
    ...
    def send_task(self, task_info: Dict):
        ...
        is_success, response = self.handle_task_result(response, task_info["task_type"], collection, task_info)
        if not is_success:
            response = None

        if not response:
            # 推送任务
            cookie = self.cm.get_valid_cookie()  # 从 cookie 池中获取一个可用 cookie
            task_info["cookie"] = cookie
            self.add_task_to_redis(queue_name, task_info)
            logger.info(f"✅ 投放任务到 {queue_name}，使用 cookie: {cookie[:20]}...")

            # 标记 cookie 正在使用，设置冷却时间
            self.cm.mark_cookie_used(cookie)
        else:
            return response
        # 获取任务结果， 2分钟抛出超时错误
        ...
        while True:
            ...
            if time.time() - start_time > timeout:
                raise TimeoutError("获取任务结果超时，已超过2分钟")
        ...
        # 检查响应是否正常
        is_success, result = self.handle_task_result(result, task_info["task_type"], collection, task_info, cookie=cookie)
    ...
    def handle_task_result(self, result: dict, task_type: str, collection: str, task_info, cookie: str = None):
        cookie_error = 0
        result_str = json.dumps(result)

        if result.get('msg') =="搜索不到酒店":
            return True, result
        if "priceInfo" in result_str:
            if task_type == "XC_ROOM_DETAIL_RP_PIC_DISCOUNT":
                if "totalPriceInfo" in result_str:
                    ...
                    if cookie:
                        self.stat_cookie(cookie, cookie_error)
                    return True, result
                else:
                    cookie_error = 1
            else:
                if "tipAfterPrice" in result_str:
                    logger.info("✅ cookie 正常，任务处理完成。")
                    if cookie:
                        self.stat_cookie(cookie, cookie_error)
                    return True, result
        ...
        if cookie_error and cookie:

            result = {}
            logger.warning(f"⚠️ 检测到 cookie 失效，移除中... {cookie[:20]}...")
            check_in = task_info.get("check_in")
            check_out = task_info.get("check_out")
            hotel_name = task_info.get("hotel_name")
            today = datetime.datetime.now().strftime("%Y-%m-%d")
            query = {"hotel_name": hotel_name, "check_in": check_in, "check_out": check_out, "date": today}
            self.mongo.delete_many(collection, query=query)
            self.cm.remove_invalid_cookie(cookie)
        return False, result
    ...
    def run(self):
        ...
        with self.lock:  # 确保单个账号串行执行
            ...
            while True:
                ...
                if "搜索不到酒店" in json.dumps(response):
...
class MultiAccountScheduler:
    ...
    def _run_scheduler_wrapper(self, scheduler):
        ...
        try:
            ...
            logger.error(f"账号 {scheduler.username} 运行时发生异常: {e}")
            raise
