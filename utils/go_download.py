...
PROXY_ERROR_PATTERN = re.compile(r"proxyconnect tcp:.*connection refused")
config_path = '../config/tls_configs.json'
with open(config_path, 'r', encoding='utf-8') as f:
    tls_config = json.load(f)

    tls_configs = tls_config["chrome"]
class Downloader(AbstractDownloader):
    ...
    def __init__(
        self,
        tls_config: list[Union[dict, TLSConfig]] = None,
        options: Optional[dict] = None,
    ) -> None:
        self.tls_config = tls_config or tls_configs
    ...
    def fetch(self, request: Request) -> Response:
        ...
        if not tls_config:
            tls_config = random.choice(tls_configs)
...
if __name__ == "__main__":
    ...
    headers = {
        ...
        "Referer": "https://pubs.acs.org/loi/aaembp",
        "Origin": "https://pubs.acs.org",
        "User-Agent": "@chrome"
    }
    rsp = downloader.fetch(
        Request(url="https://pubs.acs.org/loi/aaembp", headers=headers, timeout=20)
    )
